...
def _find_candidates(D, V, B_set, print_info):
    ...
    for x, y, z in permutations(V, 3):
        ...
        if z in B_set:
            # elements in the set B should not be reduced as z (WP 3)
            continue
...
def modified_recognize(D, B_set, first_candidate_only=False, print_info=False):
    """Recognition of type R matrices.
    ...
    D : 2-dimensional numpy array
        A distance matrix.
    B_set: list of integers
        set B as of WP3 from Praktikumsbeschreibung.    
        given an empty set, modified_recognize should behave identical to recognize from Erdbeermet
    first_candidate_only : bool, optional
        If True, only consider the first found candidate for a merge event.
        The default is False.
    print_info : bool, True
    ...
    while stack:
        ...
        if n > 4:
            ...
            candidates = _find_candidates(D, V, B_set, print_info)
